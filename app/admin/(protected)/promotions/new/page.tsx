...
export default function NewPromotionPage() {
  ...
  const [triggerType, setTriggerType] = useState("MIN_CART_VALUE");
  const [rewardType, setRewardType] = useState("PERCENTAGE_DISCOUNT");
  ...
  return (
    <form onSubmit={handleSubmit} style={{ paddingBottom: "4rem", animation: "fadeIn 0.5s ease" }}>
      <div style={{ marginBottom: "2rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h1 style={{ fontSize: "1.5rem", fontWeight: 400, color: "#1a1a18", margin: "0 0 0.25rem 0" }}>Create Promotion</h1>
          <p style={{ fontSize: "0.85rem", color: "#6b6865", margin: 0 }}>Configure discount rules, audience eligibility, and assignments.</p>
        </div>
          ...
          <section style={{ background: "#ffffff", border: "1px solid #e8e4df", borderRadius: "2px" }}>
            ...
            <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
              <div>
                ...
                <input name="name" required type="text" placeholder="e.g. Summer VIP Exclusive" style={{ width: "100%", padding: "0.8rem", border: "1px solid #e8e4df", fontSize: "0.85rem", borderRadius: "2px" }} />
              </div>
              
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
                <div>
                  <label style={{ display: "block", fontSize: "0.75rem", color: "#6b6865", marginBottom: "0.5rem" }}>Discount Code (Optional)</label>
                  <input name="code" type="text" placeholder="e.g. SUMMER20" style={{ width: "100%", padding: "0.8rem", border: "1px solid #e8e4df", fontSize: "0.85rem", fontFamily: "var(--font-dm-mono, monospace)", borderRadius: "2px" }} />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
                ...
                {triggerType !== 'NO_TRIGGER' && triggerType !== 'SPECIFIC_PRODUCTS' && (
                  <div>
                    ...
                    <input name="triggerValue" required type="number" placeholder={triggerType === 'MIN_CART_VALUE' ? "e.g. 5000 (₹)" : "e.g. 3 (items)"} style={{ width: "100%", padding: "0.8rem", border: "1px solid #e8e4df", fontSize: "0.85rem", borderRadius: "2px" }} />
                  </div>
                )}
              </div>
            </div>
          ...
          <section style={{ background: "#ffffff", border: "1px solid #e8e4df", borderRadius: "2px" }}>
            ...
            <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
              <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.85rem", color: "#1a1a18" }}>
                <input name="firstOrderOnly" type="checkbox" /> Valid for first-time orders only
              </label>
            </div>
          </section>
          ...
          <section style={{ background: "#ffffff", border: "1px solid #e8e4df", borderRadius: "2px" }}>
            ...
            <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
              <div>
                ...
                <input name="validFrom" type="date" style={{ width: "100%", padding: "0.8rem", border: "1px solid #e8e4df", fontSize: "0.85rem", borderRadius: "2px" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: "0.75rem", color: "#6b6865", marginBottom: "0.5rem" }}>End Date (Optional)</label>
                <input name="validUntil" type="date" style={{ width: "100%", padding: "0.8rem", border: "1px solid #e8e4df", fontSize: "0.85rem", borderRadius: "2px" }} />
              </div>
            </div>
          </section>
        </div>
